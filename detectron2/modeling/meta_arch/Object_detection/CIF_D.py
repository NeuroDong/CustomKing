#Object detection network based on channel internal feature and Multi-head attention module
...
class CIF_D(DenseDetector):
    ...
    def from_config(cls, cfg):
        ...
        backbone_shape = [backbone.output_shape()["CIF_MultiHead"]]
        head = CIF_head(cfg,backbone_shape)
        anchor_generator = build_anchor_generator(cfg, backbone_shape)
        return {
            "backbone": backbone,
            "head": head,
            ...
            "num_classes": cfg.CIF.NUM_CLASSES,
...
class CIF_head(nn.Module):
    """
    The head used in CIF_MultiHead for object classification and box regression.
    ...
    def __init__(
        ...
        *,
        ...
        self.cls_subnet = PoswiseFeedForwardNet()
        self.bbox_subnet = PoswiseFeedForwardNet()
        self.cls_score = nn.Conv2d(
            input_shape[0].channels, num_anchors * num_classes, kernel_size=3, stride=1, padding=1
        )
        self.bbox_pred = nn.Conv2d(
            input_shape[0].channels, num_anchors * 4, kernel_size=3, stride=1, padding=1
        )
        
    @classmethod
    def from_config(cls, cfg, input_shape: List[ShapeSpec]):
        num_anchors = build_anchor_generator(cfg, input_shape).num_cell_anchors #len(ANCHOR_GENERATOR.SIZES)*len(ANCHOR_GENERATOR.ASPECT_RATIOS)
    ...
    def forward(self, x:Tensor):
        """
        Arguments:
            x(Tensor): CIF_MultiHead feature map tensors in high to low resolution.
        ...
        """
        x = x[0]
        return [self.cls_score(self.cls_subnet(x))],[self.bbox_pred(self.bbox_subnet(x))]
