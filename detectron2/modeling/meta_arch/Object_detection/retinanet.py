...
class RetinaNet(DenseDetector):
    ...
    @configurable  #负责将cfg里面的参数传给from_config函数，并在from_config函数里面对参数进行处理，再调用__init__函数。
    def __init__(
        self,
        *,
        backbone: Backbone,
...
class RetinaNetHead(nn.Module):
    ...
    def forward(self, features: List[Tensor]):
        ...
        for feature in features:
            ...
            print(self.cls_score(self.cls_subnet(feature)).shape)
            bbox_reg.append(self.bbox_pred(self.bbox_subnet(feature)))
            print(self.bbox_pred(self.bbox_subnet(feature)).shape)
